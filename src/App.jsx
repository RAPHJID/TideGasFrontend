...
const API = "https://localhost:7289";
...
export default function App() {
  ...
  async function handleLogin(e) {
    ...
    try {
      ...
      setUser({ email: data.user.email, roles: data.roles });
    } catch (err) {
  ...
  const css = `
    * { box-sizing: border-box; margin: 0; padding: 0; }
    ...
    .badge { background: #f2f2f0; border: 0.5px solid #e0e0dc; border-radius: 5px; font-size: 11px; font-weight: 500; padding: 3px 8px; color: #555; text-transform: uppercase; letter-spacing: 0.4px; margin: 2px; display: inline-block; }
        ...
        <div className="card">
          ...
          {user ? (
            <div className="center">
              <p style={{ fontSize: 13, color: "#999", marginBottom: 4 }}>Signed in as</p>
              <p style={{ fontSize: 15, fontWeight: 500, color: "#111", marginBottom: 12 }}>{user.email}</p>
              <div style={{ marginBottom: "1.5rem" }}>
                {user.roles?.map(r => <span key={r} className="badge">{r}</span>)}
              </div>
