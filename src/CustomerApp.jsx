...
import { apiFetch, CUSTOMER_API, getRoles, getUserEmail, isAdmin, isAdminOrStaff } from "./config";
    ...
    <div className="modal-bg" onClick={e => e.target === e.currentTarget && onClose()}>
      <div className="modal">
        ...
        <form onSubmit={handleSubmit}>
          ...
          <div className="field"><label>Phone number</label><input value={form.phoneNumber} onChange={e => set("phoneNumber", e.target.value)} placeholder="+974 1234 5678" /></div>
          ...
          <div className="modal-actions">
            ...
            <button type="submit" className="btn btn-dark" disabled={loading}>{loading ? "Saving…" : isEdit ? "Save changes" : "Add customer"}</button>
          </div>
        </form>
      </div>
    </div>
...
export default function CustomerApp({ onLogout }) {
  ...
  async function load() {
    ...
    try { const data = await apiFetch(CUSTOMER_API, "/api/customer"); setCustomers(data || []); }
    catch (err) { setError(err.message); }
    finally { setLoading(false); }
  }
  ...
  async function handleDelete(id) {
    ...
    try { await apiFetch(CUSTOMER_API, `/api/customer/${id}`, { method: "DELETE" }); setSuccess("Customer deleted."); setTimeout(() => setSuccess(""), 2500); load(); }
    catch (err) { setError(err.message); }
  }
    ...
    <>
      ...
      <div className="app">
        <div className="topbar">
          <div className="topbar-left"><h1>Customer Management</h1><p>{userEmail}{roles.map(r => <span key={r} className="role-badge">{r}</span>)}</p></div>
          <div className="topbar-right">
            <button className="btn" onClick={load}>Refresh</button>
            {isAdminOrStaff() && <button className="btn btn-dark" onClick={() => { setSelected(null); setModal("add"); }}>+ Add customer</button>}
            <button className="btn" onClick={onLogout}>Sign out</button>
          </div>
        </div>
        <div className="stat-row">
          <div className="stat"><div className="stat-label">Total customers</div><div className="stat-value">{customers.length}</div></div>
        </div>
        {error && <div className="error-box">{error}</div>}
        ...
        {loading ? <div className="loading"><div className="spinner" /> Loading customers…</div>
          : filtered.length === 0 ? <div className="empty">{search ? "No customers match your search." : "No customers yet."}</div>
          : (
            <div className="table-wrap">
              <table>
                <thead><tr><th>Customer</th><th>Phone</th><th>Address</th>{isAdminOrStaff() && <th>Actions</th>}</tr></thead>
                <tbody>
                  {filtered.map(c => (
                    <tr key={c.id}>
                      <td><div className="name-cell">
                        ...
                        <div>
                          <div className="name" style={{ cursor: "pointer", textDecoration: "underline", textUnderlineOffset: 2 }} onClick={() => { setSelected(c); setModal("detail"); }}>{c.fullName}</div>
