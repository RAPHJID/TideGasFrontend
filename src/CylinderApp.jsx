...
const CYLINDER_API = "https://localhost:7139"; // CylinderService port
...
async function apiFetch(path, options = {}) {
  ...
  const res = await fetch(`${CYLINDER_API}${path}`, {
    ...options,
    headers: {
...
function statusStyle(status) {
  return STATUS_COLORS[status] || { bg: "#f2f2f0", border: "#e0e0dc", color: "#555" };
}
...
const css = `
  * { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  .tab { padding: 7px 16px; font-size: 13px; font-weight: 500; border: none; border-radius: 7px; cursor: pointer; background: transparent; color: #999; font-family: inherit; }
...
function CylinderModal({ cylinder, onClose, onSaved }) {
  ...
  const [form, setForm] = useState({
    ...
    pricePerUnit: cylinder?.pricePerUnit || "",
  });
  ...
  async function handleSubmit(e) {
    e.preventDefault();
    setError("");
    if (!form.serialNumber || !form.size || !form.gasType || !form.pricePerUnit) {
    ...
    <div className="modal-bg" onClick={e => e.target === e.currentTarget && onClose()}>
      <div className="modal">
        ...
        <form onSubmit={handleSubmit}>
          <div className="field"><label>Serial number</label><input value={form.serialNumber} onChange={e => set("serialNumber", e.target.value)} placeholder="CYL-001" /></div>
          <div className="field"><label>Size (kg)</label><input value={form.size} onChange={e => set("size", e.target.value)} placeholder="12.5" /></div>
          <div className="field"><label>Gas type</label><input value={form.gasType} onChange={e => set("gasType", e.target.value)} placeholder="LPG" /></div>
          <div className="field">
            <label>Status</label>
          ...
          <div className="field"><label>Price per unit (QAR)</label><input value={form.pricePerUnit} onChange={e => set("pricePerUnit", e.target.value)} placeholder="55.00" type="number" /></div>
  ...
  return (
    <div className="modal-bg" onClick={e => e.target === e.currentTarget && onClose()}>
      <div className="modal">
        ...
        <p style={{ fontSize: 13, color: "#999", marginBottom: "1.25rem" }}>{cylinder.serialNumber} — {cylinder.gasType}</p>
        {error && <div className="error-box">{error}</div>}
        <form onSubmit={handleSubmit}>
  ...
  return (
    <div className="card">
      <div className="card-header">
        <div>
          <div className="card-title">{cylinder.serialNumber}</div>
          <div className="card-sub">{cylinder.gasType}</div>
        </div>
      ...
      <div className="meta">
        <div className="meta-row"><span>Size</span><span>{cylinder.size} kg</span></div>
        <div className="meta-row"><span>Price</span><span>QAR {Number(cylinder.pricePerUnit).toFixed(2)}</span></div>
      ...
      <div className="card-actions">
        {isAdminOrStaff() && (
          <button className="btn" style={{ fontSize: 12, padding: "6px 12px" }} onClick={() => onSales(cylinder)}>Daily sales</button>
        )}
        {isAdmin() && (
          <>
            <button className="btn" style={{ fontSize: 12, padding: "6px 12px" }} onClick={() => onEdit(cylinder)}>Edit</button>
            <button className="btn btn-danger" style={{ fontSize: 12, padding: "6px 12px" }} onClick={() => onDelete(cylinder.id)}>Delete</button>
          </>
        )}
      </div>
    </div>
  ...
  return (
    <>
      ...
      <div className="app">
        ...
        <div className="topbar">
          ...
          <div className="topbar-right">
            ...
            {isAdmin() && (
              <button className="btn btn-dark" onClick={() => { setSelected(null); setModal("add"); }}>+ Add cylinder</button>
            )}
      ...
      {(modal === "add" || modal === "edit") && (
        <CylinderModal cylinder={modal === "edit" ? selected : null} onClose={() => setModal(null)} onSaved={handleSaved} />
      )}
      {modal === "sales" && selected && (
        <DailySalesModal cylinder={selected} onClose={() => setModal(null)} onSaved={handleSaved} />
      )}
    </>
  );
}
