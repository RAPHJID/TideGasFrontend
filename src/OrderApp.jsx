...
import { apiFetch, ORDER_API, CUSTOMER_API, CYLINDER_API, getRoles, getUserEmail, isAdmin, isAdminOrStaff } from "./config";
...
function statusStyle(s) { return STATUS_STYLES[s] || { bg: "#f2f2f0", border: "#e0e0dc", color: "#555" }; }
...
function CreateOrderModal({ onClose, onSaved, customers, cylinders }) {
  ...
  async function handleSubmit(e) {
    ...
    if (!form.customerId || !form.cylinderId || !form.quantity || !form.totalPrice) { setError("Please fill in all fields."); return; }
    setLoading(true);
    try {
    ...
    <div className="modal-bg" onClick={e => e.target === e.currentTarget && onClose()}>
      <div className="modal">
        ...
        <form onSubmit={handleSubmit}>
          <div className="field"><label>Customer</label><select value={form.customerId} onChange={e => set("customerId", e.target.value)}><option value="">Select a customer…</option>{customers.map(c => <option key={c.id} value={c.id}>{c.fullName} — {c.email}</option>)}</select></div>
          <div className="field"><label>Cylinder</label><select value={form.cylinderId} onChange={e => set("cylinderId", e.target.value)}><option value="">Select a cylinder…</option>{cylinders.map(c => <option key={c.id} value={c.id}>{c.brand} — {c.size} ({c.status})</option>)}</select></div>
          ...
          <div className="modal-actions">
            ...
            <button type="submit" className="btn btn-dark" disabled={loading}>{loading ? "Creating…" : "Create order"}</button>
          </div>
        </form>
      </div>
    </div>
    ...
    <div className="modal-bg" onClick={e => e.target === e.currentTarget && onClose()}>
      <div className="modal">
        ...
        <div style={{ marginBottom: "1.25rem" }}>
          <div className="detail-row"><span>Order ID</span><span style={{ fontSize: 11, color: "#999" }}>{order.id}</span></div>
          ...
          <div className="detail-row"><span>Status</span><span className="badge" style={{ background: st.bg, borderColor: st.border, color: st.color }}>{order.status}</span></div>
          <div className="detail-row"><span>Created</span><span>{new Date(order.createdAt).toLocaleString()}</span></div>
        </div>
        <div className="modal-actions">
          <button className="btn" onClick={onClose}>Close</button>
          {isAdmin() && <button className="btn btn-danger" onClick={() => { onClose(); onDelete(order.id); }}>Delete</button>}
        </div>
      </div>
    </div>
  );
...
export default function OrderApp({ onLogout }) {
  ...
  async function load() {
    ...
    try {
      ...
      setOrders(ordersData || []); setCustomers(customersData || []); setCylinders(cylindersData || []);
    } catch (err) { setError(err.message); }
    finally { setLoading(false); }
  }
  ...
  async function handleDelete(id) {
    ...
    try { await apiFetch(ORDER_API, `/api/Order/${id}`, { method: "DELETE" }); setSuccess("Order deleted."); setTimeout(() => setSuccess(""), 2500); load(); }
    catch (err) { setError(err.message); }
  }

  function handleSaved() { setModal(null); setSuccess("Order created successfully."); setTimeout(() => setSuccess(""), 2500); load(); }

  const filtered = orders.filter(o => {
    const matchSearch = o.customerName?.toLowerCase().includes(search.toLowerCase()) || o.cylinderName?.toLowerCase().includes(search.toLowerCase()) || o.id?.toLowerCase().includes(search.toLowerCase());
    ...
    <>
      ...
      <div className="app">
        <div className="topbar">
          <div className="topbar-left"><h1>Order Management</h1><p>{userEmail}{roles.map(r => <span key={r} className="role-badge">{r}</span>)}</p></div>
          <div className="topbar-right">
            <button className="btn" onClick={load}>Refresh</button>
            {isAdminOrStaff() && <button className="btn btn-dark" onClick={() => setModal("create")}>+ New order</button>}
        ...
        <div className="search-bar">
          <input placeholder="Search by customer, cylinder or order ID…" value={search} onChange={e => setSearch(e.target.value)} />
        ...
        {loading ? <div className="loading"><div className="spinner" /> Loading orders…</div>
          : filtered.length === 0 ? <div className="empty">{search || statusFilter !== "all" ? "No orders match your search." : "No orders yet."}</div>
          : (
            <div className="table-wrap">
              <table>
                <thead><tr><th>Customer</th><th>Cylinder</th><th>Qty</th><th>Total</th><th>Status</th><th>Date</th>{isAdminOrStaff() && <th>Actions</th>}</tr></thead>
                <tbody>
                  {filtered.map(o => {
                    const st = statusStyle(o.status);
                    return (
                      <tr key={o.id}>
                        <td><span style={{ cursor: "pointer", textDecoration: "underline", textUnderlineOffset: 2, fontWeight: 500 }} onClick={() => { setSelected(o); setModal("detail"); }}>{o.customerName}</span></td>
                        ...
                        <td>
                          {isAdminOrStaff()
                            ? <select className="status-select" value={o.status} style={{ background: st.bg, color: st.color, borderColor: st.border }} onChange={e => handleStatusChange(o.id, e.target.value)}><option>Pending</option><option>Processing</option><option>Completed</option><option>Cancelled</option></select>
                            : <span className="badge" style={{ background: st.bg, borderColor: st.border, color: st.color }}>{o.status}</span>
                          }
                        </td>
                        <td style={{ color: "#999", fontSize: 12 }}>{new Date(o.createdAt).toLocaleDateString()}</td>
                        {isAdminOrStaff() && <td><div className="actions">{isAdmin() && <button className="btn btn-danger" style={{ fontSize: 12, padding: "5px 10px" }} onClick={() => handleDelete(o.id)}>Delete</button>}</div></td>}
                      </tr>
                    );
                  })}
                </tbody>
      ...
      {modal === "detail" && selected && <OrderDetail order={selected} onClose={() => setModal(null)} onDelete={handleDelete} />}
    </>
  );
}
